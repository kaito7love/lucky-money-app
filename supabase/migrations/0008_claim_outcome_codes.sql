-- claim_envelope() rejected every non-'active' pool with a flat POOL_CLOSED,
-- which 0003 recorded as deliberate when it added the 'closed' status. The
-- consequence only became visible from the guest's side: the function itself
-- sets status = 'completed' the moment the last envelope goes, so a guest who
-- arrives seconds late trips the status check before ever reaching the
-- NO_ENVELOPES_LEFT branch below it. That branch was unreachable in every
-- ordinary run, and its far kinder wording -- "Đã hết bao lì xì, chúc bạn năm
-- sau may mắn hơn!" -- never once reached anyone. An expired pool told the
-- same story, so "the host shut this down", "you were a moment too late" and
-- "the deadline passed" all read identically.
--
-- The statuses already distinguish all three; only this check flattened them.
-- Reported apart, each ending gets the words it deserves. The already-claimed
-- check moves above them for the same reason.
